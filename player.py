import pygame

from anim import Anim
from pictures_and_any import player_image, tile_width, tile_height
from settings import *
from weapons import Stick


class Player(Anim):
    def __init__(self, pos_x: int, pos_y: int, hp: int = 100, speed: int = 12):
        self.sheet: pygame.Surface = player_image
        self.columns: int = 4
        self.rows: int = 2
        super(Player, self).__init__(self.sheet, self.columns, self.rows, pos_x, pos_y, speed, hp)
        self.rect = self.image.get_rect().move(
            tile_width * pos_x + 15, tile_height * pos_y + 5)
        self.set_weapon(Stick())

    def move(self, dx: int, dy: int):
        super(Player, self).move(dx, dy)
        if self.weapon is not None:
            self.weapon.move(dx * self.speed, dy * self.speed)

        if pygame.sprite.spritecollideany(self, walls_group):
            super(Player, self).move(-dx, -dy)
            if self.weapon is not None:
                self.weapon.move(-dx * self.speed, -dy * self.speed)
